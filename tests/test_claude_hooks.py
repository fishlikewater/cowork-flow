...
class ClaudeHooksTest(unittest.TestCase):
    ...
    def test_hook_reads_utf8_prompt_bytes_before_classification(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            self._write_runtime_session(root, "claude_demo-session", ".cowork-flow/tasks/06-04-demo")
    ...
    def test_hook_resolves_active_task_from_claude_session_id(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            task_dir.mkdir(parents=True)
            (task_dir / "task.json").write_text('{"status": "in_progress"}\n', encoding="utf-8")
            self._write_runtime_session(root, "claude_demo-session", ".cowork-flow/tasks/06-03-demo")
    ...
    def test_hook_reads_flow_only_active_task_status(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            task_dir = root / ".cowork-flow" / "tasks" / "06-03-demo"
            task_dir.mkdir(parents=True)
            self._write_flow_task(root, artifact_dir="06-03-demo", status="review")
            self._write_runtime_session(root, "claude_demo-session", ".cowork-flow/tasks/06-03-demo")
