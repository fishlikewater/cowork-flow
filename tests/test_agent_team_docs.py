...
class AgentTeamDocsTest(unittest.TestCase):
    ...
    def test_agent_team_execution_skill_uses_codex_subagent_orchestration_language(self) -> None:
        for path in (
            ...
            self.assertIn("Do not request a full-history fork", skill)
            self.assertIn("full-history fork cannot be combined with `agent_type`", skill)
            self.assertIn("fresh child thread with explicit prompt context", skill)
