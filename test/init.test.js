...
test('init copies the template into a new target directory', async (t) => {
  ...
  assert.equal(await exists(join(target, '.cowork-flow', 'scripts', 'common', 'task_context_defaults.py')), true);
  ...
  assert.equal(await exists(join(target, '.cowork-flow', 'spec', 'core', 'state-templates.md')), true);
