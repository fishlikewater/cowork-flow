...
- The `.context.json` file is assignment-scoped context: it names the worker brief and `allowedContext`, the minimal task PRD/context indexes/global rules the worker may load for this assignment.
...
- `worker-report` is accepted only when the assignment state is ready or in_progress; pending workers must report `NEEDS_CONTEXT` to the coordinator instead of writing an outbox early.
