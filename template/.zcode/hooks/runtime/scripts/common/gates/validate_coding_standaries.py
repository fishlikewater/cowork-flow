#!/usr/bin/env python3
"""Spec-driven coding standards (zcode hooks runtime).

ZCode-flavored copy: relative imports (``.coding_standaries``), same CLI
interface as the original (--task-dir required). Behaves identically to
``template/.cowork-flow/scripts/common/gates/validate_coding_standaries.py``.
"""
...
# Standalone-CLI import fallback: when loaded directly (not as a
# package), the relative imports cannot resolve. Patch ``__package__``
# and re-add this directory to ``sys.path`` so the absolute-style
# fallbacks work identically to the relative ones.
if __name__ == "__main__" and __package__ in (None, ""):
    import sys as _sys
    _dir = Path(__file__).resolve().parent
    _parent = _dir.parent          # common/gates -> common
    _grand = _parent.parent        # common
    if str(_grand) not in _sys.path:
        _sys.path.insert(0, str(_grand))
    if str(_parent) not in _sys.path:
        _sys.path.insert(0, str(_parent))
    __package__ = "common.gates"
...
def get_coding_standaries_summary(
    repo_root: Path,
    task_dir: Path,
) -> str:
...
def validate_coding_standaries(
    repo_root: Path,
    task_dir: Path,
) -> list[dict]:
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description="Spec-driven coding gate (zcode)")
    ...
    if args.list:
        ...
        return
    if args.summarize:
        print(get_coding_standaries_summary(repo_root, task_dir) or "No spec rules activated.")
        return
    if args.validate:
        v = validate_coding_standaries(repo_root, task_dir)
        for x in v:
