...
Read `AGENTS.md` and `.trellis/workflow.md` for the project's commit policy. The scripts below handle their own commits for `.trellis/` metadata when git is available; do not use this skill to decide who commits business code.
...
If the current task uses a plan file in `docs/superpowers/plans/`, update it before recording the session:
...
- [OK] Auto-commits .trellis/workspace and .trellis/tasks changes when git is available
