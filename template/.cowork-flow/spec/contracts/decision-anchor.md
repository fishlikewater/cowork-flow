...
`## 验证命令` 与 `## 范围边界` 会被 stage-contract 注入与 MCP 事实视图解析（`services/fact_view.parse_decision_anchor`）。行格式约定：

- `## 验证命令`：每行一条命令（可带 `- ` 前缀），截 120 字符，最多 5 条；命令由实现 agent 自行执行（自声明自查），reviewer 核对执行证据
- `## 范围边界`：仅取首个非空行，截 160 字符
