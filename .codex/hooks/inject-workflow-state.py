...
def _build_host_block(root: Path, dispatch_mode: str) -> str:
...
def main() -> int:
    ...
    runtime_context, runtime_context_id = resolve_runtime_context(root, hook_input)
    ...
    additional_context = build_context(
        ...
        source,
        breadcrumbs,
        lambda: _build_host_block(root, dispatch_mode),
        extra_lines,
    )
    return emit_hook_output("UserPromptSubmit", additional_context)
